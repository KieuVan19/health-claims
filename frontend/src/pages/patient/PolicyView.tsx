    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        {policies.map((policy) => {
          ...
          return (
            <div key={policy.id} className="card overflow-hidden" data-testid="policy-card">
              {/* Header */}
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-5 text-white">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${policyTypeColors[policy.type]}`}>
                      {policy.type}
                    </span>
              ...
              <div className="p-5">
                ...
                <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl mb-4" data-testid="coverage-progress-section">
                  ...
                  <div className="w-full bg-blue-200 rounded-full h-3" data-testid="coverage-progress-bar">
                    <div
                ...
                <div className="grid grid-cols-3 gap-4 mb-5">
                  <div className="text-center p-4 bg-gray-50 rounded-xl">
                    ...
                    {summary ? (
                      ...
                      <p className="text-base font-bold text-gray-900">{formatCurrency(policy.deductible)}</p>
                    )}
                  </div>
                  <div className="text-center p-4 bg-gray-50 rounded-xl">
                    <DollarSign className="h-6 w-6 text-green-600 mx-auto mb-1" />
                    <p className="text-sm text-gray-500">Copay</p>
                    <p className="text-base font-bold text-gray-900">{policy.copayPercentage ?? 0}%</p>
                  </div>
                  <div className="text-center p-4 bg-gray-50 rounded-xl">
                    ...
                    {summary ? (
                      ...
                      <p className="text-base font-bold text-gray-900">{formatCurrency(policy.oopMax ?? 8000)}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-4 mb-5 pb-5 border-b border-gray-100">
                  <div className="flex items-center gap-2 text-base text-gray-600">
                    ...
                    <span>Effective: <strong>{format(new Date(policy.effectiveDate), 'MMM d, yyyy')}</strong></span>
                  </div>
                  <div className="flex items-center gap-2 text-base text-gray-600">
                    <Calendar className="h-5 w-5 text-red-400" />
                    <span>Expires: <strong className={isExpiringSoon ? 'text-amber-600' : ''}>{format(expiryDate, 'MMM d, yyyy')}</strong></span>
                  </div>
                </div>
